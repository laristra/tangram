...
#include "wonton/wonton/mesh/jali/jali_mesh_wrapper.h"
...
#include "tangram/utility/advance_moments.h"

/* Demo app for the 3D Rotor example.
   Uses Jali, VOF, and MOF.
   Generates or reads mesh from file, computes material moments and reference poly's
...
int main(int argc, char** argv) {
  ...
  if ((argc != 3) && (argc != 5)) {
    ...
    "Correct usage: " << std::endl << 
      "  for structured mesh: rotor_demo_jali" << 
      " <decompose_cells> [0|1]" <<
      " <nx> <ny> <nz> " << std::endl << 
      "for Exodus II mesh: rotor_demo_jali" << 
      " <decompose_cells> [0|1]" <<
  ...
  if(argc == 3)
    mesh = mesh_factory(argv[2]);
  else
    mesh = mesh_factory(0.25, 0.1, 0.25,
                        0.75, 0.9, 0.75,
                        nx, ny, nz);
  ...
  std::cout << "Time taken to generate/read mesh -> " << 
