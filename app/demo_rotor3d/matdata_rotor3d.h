...
planetary_gear(const std::vector< std::shared_ptr<RefPolyData_t> >& polys_data,
  ...
  std::vector<Tangram::Point3> sun_pts = 
    cog3d(gears_center, sun_in_rad, sun_out_rad, normal, sun_nteeth);    
  ...
  std::vector<Tangram::Point3> sun_shaft_pts = 
    circle3d(frame_center, sun_shaft_rad, normal, shaft_nsides);
  Tangram::MatPoly<3> sun_shaft_poly = prism(sun_shaft_pts, 
    2*carrier_clearance + carrier_depth + gears_depth, 1.0, normal);

  Tangram::MatPoly<3> sun_poly = prism(sun_pts, gears_depth, 1.0, normal);
  ...
  for (int iplanet = 0; iplanet < nplanets; iplanet++) {
    ...
    planet_pts = cog3d(planet_centers[iplanet], planet_rad - 0.5*tooth_depth, 
      planet_rad + 0.5*tooth_depth, normal, planet_nteeth);
    ...
    planet_polys[iplanet] = prism(planet_pts, gears_depth, 1.0, normal);
    ...
    planet_shafts[iplanet] = prism(planet_shaft_pts, 
      gears_depth + 2*(carrier_clearance + carrier_depth), 1.0, normal);
    ...
    planet_shaft_gaskets[iplanet] = prism(planet_shaft_gasket_pts, gears_depth, 1.0, normal);
    ...
    planet_holes[iplanet] = prism(planet_hole_pts, gears_depth, 1.0, normal);
  }
  ...
  Tangram::MatPoly<3> carrier_polys[2];
  {
    ...
    for (int icarrier = 0; icarrier < 2; icarrier++)
      carrier_pts[icarrier] = 
        cog3d(carrier_centers[icarrier], sun_out_rad, ring_out_rad, normal, nplanets);
    ...
    for (int icarrier = 0; icarrier < 2; icarrier++)
      carrier_polys[icarrier] = prism(carrier_pts[icarrier], carrier_depth, 1.0, normal);
  }

  Tangram::MatPoly<3> carrier_cen_polys[2];
  {
    std::vector<Tangram::Point3> carrier_cen_pts = 
      circle3d(carrier_centers[0], 0.5*(sun_in_rad + sun_shaft_rad), normal, 4*shaft_nsides);
    ...
    carrier_cen_polys[0] = prism(carrier_cen_pts, carrier_depth, 1.0, normal);

    carrier_cen_pts = circle3d(carrier_centers[1], sun_shaft_rad, normal, shaft_nsides);
    for (int ivrt = 0; ivrt < carrier_cen_pts.size(); ivrt++)
    ...
    carrier_cen_polys[1] = prism(carrier_cen_pts, 
      carrier_depth + carrier_clearance, 1.0, normal);
  }
  ...
  Tangram::MatPoly<3> ring_poly;
  {
    ...
    ring_pts = cog3d(gears_center, ring_out_rad - tooth_depth, ring_out_rad, normal, ring_nteeth);
    ...
    ring_poly = prism(ring_pts, gears_depth, 1.0, normal);
  }

  std::vector<Tangram::Point3> frame_pts = 
    circle3d(frame_center, frame_rad, normal, 4*ring_nteeth);
  Tangram::MatPoly<3> frame_poly = prism(frame_pts, frame_depth, 1.0, normal);
  ...
  Tangram::MatPoly<3> ring_box = prism(ring_box_pts, gears_depth, 1.0, normal);
  ...
  apply_poly(polys_data, frame_poly, mmpolys_data, res_poly_sets_data[0], true);

  //Cutting out sun's shaft
  apply_poly(mmpolys_data, sun_shaft_poly, res_poly_sets_data[1], rem_polys_data, true);
  ...
  for (int iplanet = 0; iplanet < nplanets; iplanet++) {
    ...
    apply_poly(mmpolys_data, planet_shafts[iplanet], 
      res_poly_sets_data[iplanet + 2], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();    
  }    
  ...
  for (int iplanet = 0; iplanet < nplanets; iplanet++) {
    ...
    apply_poly(mmpolys_data, planet_shaft_gaskets[iplanet], 
      res_poly_sets_data[nplanets + 2 + iplanet], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();    
  } 
  ...
  for (int iplanet = 0; iplanet < nplanets; iplanet++) {
    ...
    apply_poly(mmpolys_data, planet_holes[iplanet], 
      res_poly_sets_data[2*nplanets + 2 + iplanet], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();    
  } 
  ...
  for (int icarrier = 0; icarrier < 2; icarrier++) {
    ...
    apply_poly(mmpolys_data, carrier_cen_polys[icarrier], 
      res_poly_sets_data[3*nplanets + 2 + icarrier], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();    
  }     
  ...
  for (int icarrier = 0; icarrier < 2; icarrier++) {
    ...
    apply_poly(mmpolys_data, carrier_polys[icarrier], 
      res_poly_sets_data[3*nplanets + 4 + icarrier], rem_polys_data, false);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();    
  }      
...

  {
    ...
    apply_poly(mmpolys_data, ring_box, ring_box_poly_data, rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
  
    //Cutting out ring interior from the box
    apply_poly(ring_box_poly_data, ring_poly, rem_polys_data, 
      res_poly_sets_data[3*nplanets + 6], false);
  ...
  apply_poly(mmpolys_data, sun_poly, res_poly_sets_data[3*nplanets + 7], rem_polys_data, false);
  mmpolys_data = rem_polys_data;
  rem_polys_data.clear();  
  ...
  for (int iplanet = 0; iplanet < nplanets; iplanet++) {
    ...
    apply_poly(mmpolys_data, planet_polys[iplanet], 
      res_poly_sets_data[3*nplanets + 8 + iplanet], rem_polys_data, false);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();    
...
void rotor_material_moments(const Mesh_Wrapper& mesh,
  ...
  std::vector<Tangram::Point3> shaft_cen_base = 
    circle3d(shaft_centroid + Tangram::Point3(0.0, 0.5*shaft_cen_len, 0.0), 
             shaft_rad, shaft_base_normal, nshaft_sides);
  Tangram::MatPoly<3> shaft_cen_poly = 
    prism(shaft_cen_base, shaft_cen_len, 1.0, shaft_base_normal);             

  std::cout << "Preparing the central part..." << std::endl;
  int icur_ref_set = ref_poly_sets.size();
  ref_poly_sets.resize(icur_ref_set + 1);

  apply_poly(mesh_polys, shaft_cen_poly, ref_poly_sets[icur_ref_set], mmpolys_data, true);
  ...
  std::vector<Tangram::Point3> shaft_mount_base = 
    circle3d(shaft_centroid + Tangram::Point3(0.0, 0.5*blades_mount_len, 0.0), 
             shaft_mount_rad, shaft_base_normal, nshaft_mount_sides);
  Tangram::MatPoly<3> shaft_mount_poly = 
    prism(shaft_mount_base, blades_mount_len, 1.0, shaft_base_normal);   

  icur_ref_set = ref_poly_sets.size();
  ref_poly_sets.resize(icur_ref_set + 1);

  apply_poly(mmpolys_data, shaft_mount_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
  mmpolys_data = rem_polys_data;
  rem_polys_data.clear();
  sets_material_IDs.push_back(mesh_material_IDs[2]);
  ...
  std::vector<Tangram::Point3> blades_mount_base = 
    circle3d(shaft_centroid + Tangram::Point3(0.0, 0.5*blades_mount_len, 0.0), 
             blades_mount_rad, shaft_base_normal, 2*nblades);
  Tangram::MatPoly<3> blades_mount_poly = 
    prism(blades_mount_base, blades_mount_len, 1.0, shaft_base_normal);   

  icur_ref_set = ref_poly_sets.size();
  ref_poly_sets.resize(icur_ref_set + 1);

  apply_poly(mmpolys_data, blades_mount_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
  mmpolys_data = rem_polys_data;
  rem_polys_data.clear();
  ...
  for (int iblade = 0; iblade < nblades; iblade++) {
    ...
    Tangram::MatPoly<3> blade_poly = 
      prism(blade_base, blades_height, blades_base_scaling);   

    icur_ref_set = ref_poly_sets.size();
    ref_poly_sets.resize(icur_ref_set + 1);

    apply_poly(mmpolys_data, blade_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    sets_material_IDs.push_back(mesh_material_IDs[3]);
  }  
  ...
  for (int igear = 0; igear < 2; igear++) {
    ...
    std::vector<Tangram::Point3> shaft_gasket_base = 
      circle3d(gear_frame_joint_base_cen[igear], gasket_r_scaling*shaft_rad, 
        gears_base_normal, 4*nshaft_sides);
    Tangram::MatPoly<3> shaft_gasket_poly = 
      prism(shaft_gasket_base, gear_frame_joint_height[igear], 1.0, gears_base_normal);

    icur_ref_set = ref_poly_sets.size();
    ref_poly_sets.resize(icur_ref_set + 1);

    apply_poly(mmpolys_data, shaft_gasket_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    ...
    std::vector<Tangram::Point3> gear_frame_joint_base = 
      circle3d(gear_frame_joint_base_cen[igear], (2*gasket_r_scaling - 1.0)*shaft_rad, 
        gears_base_normal, 8*nshaft_sides);
    Tangram::MatPoly<3> gear_frame_joint_poly = 
      prism(gear_frame_joint_base, gear_frame_joint_height[igear], 1.0, gears_base_normal);   

    icur_ref_set = ref_poly_sets.size();
    ref_poly_sets.resize(icur_ref_set + 1);

    apply_poly(mmpolys_data, gear_frame_joint_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    ...
    gear_frame_joint_base = circle3d(gear_frame_joint_base_cen[igear], 
      gear_frame_rad[igear], gears_base_normal, ngear_frame_sides);
    gear_frame_joint_poly = prism(gear_frame_joint_base, 
      gear_frame_joint_height[igear], 1.0, gears_base_normal);   
    ...
    Tangram::MatPoly<3> gear_frame_main_poly = prism(gear_frame_main_base, 
      gear_frame_main_height[igear], 1.0, gears_base_normal); 

    std::vector< std::shared_ptr<RefPolyData_t> > planetary_gear_polys;

    apply_poly(mmpolys_data, gear_frame_main_poly, planetary_gear_polys, rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    ...
    Tangram::MatPoly<3> carrier_shaft_poly = 
      prism(carrier_shaft_pts, carrier_shaft_len, 1.0, gears_base_normal);

    icur_ref_set = ref_poly_sets.size();
    ref_poly_sets.resize(icur_ref_set + 1);

    apply_poly(mmpolys_data, carrier_shaft_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    sets_material_IDs.push_back(mesh_material_IDs[9]);
    ...
    Tangram::MatPoly<3> carrier_shaft_gasket_poly = 
      prism(carrier_shaft_gasket_base, gear_frame_outer_height[igear], 1.0, gears_base_normal);

    icur_ref_set = ref_poly_sets.size();
    ref_poly_sets.resize(icur_ref_set + 1);

    apply_poly(mmpolys_data, carrier_shaft_gasket_poly, 
      ref_poly_sets[icur_ref_set], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    sets_material_IDs.push_back(mesh_material_IDs[10]);
    ...
    Tangram::MatPoly<3> gear_frame_outer_poly = 
      prism(gear_frame_outer_base, gear_frame_outer_height[igear], 1.0, gears_base_normal);   

    icur_ref_set = ref_poly_sets.size();
    ref_poly_sets.resize(icur_ref_set + 1);

    apply_poly(mmpolys_data, gear_frame_outer_poly, ref_poly_sets[icur_ref_set], rem_polys_data, true);
    mmpolys_data = rem_polys_data;
    rem_polys_data.clear();
    sets_material_IDs.push_back(mesh_material_IDs[11]);
    ...
    gear_frame_outer_poly = prism(gear_frame_outer_base, 
      gear_frame_outer_height[igear], 1.0, gears_base_normal);   
