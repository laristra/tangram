...
#include "tangram/reconstruct/LVIRA.h"
...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    ...
    os << std::endl <<
    "Correct usage: test_lvira_2d" << " <decompose_cells> [0|1]" <<
...
// Build the driver
#if defined(WONTON_ENABLE_Jali) && defined(WONTON_ENABLE_MPI)
  Tangram::Driver<Tangram::LVIRA, 2, Wonton::Jali_Mesh_Wrapper,
                  Tangram::SplitRnD<2>, Tangram::ClipRnD<2>>
    lvira_driver(mesh_wrapper, ims_tols, !decompose_cells);
#else
  Tangram::Driver<Tangram::LVIRA, 2, Wonton::Simple_Mesh_Wrapper,
                  Tangram::SplitRnD<2>, Tangram::ClipRnD<2>>
    lvira_driver(mesh_wrapper, ims_tols, !decompose_cells);
#endif
