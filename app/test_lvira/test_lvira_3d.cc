...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    ...
    os << std::endl <<
    "Correct usage: test_lvira_3d" << " <decompose_cells> [0|1]" <<
  ...
  // Build the driver
#if defined(WONTON_ENABLE_Jali) && defined(WONTON_ENABLE_MPI)
  Tangram::Driver<Tangram::LVIRA, 3, Wonton::Jali_Mesh_Wrapper,
                  Tangram::SplitR3D, Tangram::ClipR3D>
    lvira_driver(mesh_wrapper, ims_tols, !decompose_cells);
#else
  Tangram::Driver<Tangram::LVIRA, 3, Wonton::Simple_Mesh_Wrapper,
                  Tangram::SplitR3D, Tangram::ClipR3D>
    lvira_driver(mesh_wrapper, ims_tols, !decompose_cells);
#endif
