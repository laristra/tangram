...
TEST(Matrix, MatVecMult) {
  ...
  Tangram::Matrix B({{1, 2.5, -1},
                     {2, -3, -2},
                     {0, 1.1, 2.3}});

  // Check multiplication with Tangram::Vector
  Tangram::Vector<3> Y, BY;
...
TEST(Matrix, MatMatMult) {
  ...
  Tangram::Matrix B({{3.0, 2.0, 1.0},
                     {2.0, -3.0, -5.0},
                     {2.5, 2.6, 0.0},
                     {5.5, -2.0, 3.5}});

  Tangram::Matrix AB_expected({{22, -14.1, -1},
                               {0.5, 5.8, 20.5},
                               {-8.55, 8.68, -16.0}});

  
  Tangram::Matrix AB = A*B;
...
TEST(Matrix, MatInverse) {
  ...
  Tangram::Matrix Ainv = A.inverse();

  Tangram::Matrix Ainv_expected({{0.213592, 0.097087, 0.048544, -0.296117},
