...
#include "tangram/wrappers/mesh/AuxMeshTopology.h"
...
/*!
  ...
  Jali_Mesh_Wrapper implements methods required for Tangram mesh
...
//! helper function:  convert Jali point to Tangram point
template <long D>
Point<D> toTangramPoint(const JaliGeometry::Point& jp) {
  Point<D> pp;
  assert(jp.dim() == D);
  for (int d = 0; d < D; ++d)
    pp[d] = jp[d];
...
class Jali_Mesh_Wrapper : public AuxMeshTopology<Jali_Mesh_Wrapper> {
 public:
  ...
  Tangram::Entity_type cell_get_type(int const cellid) const {
    ...
    return jali2tangram_type(etype);
  }
  ...
  Tangram::Element_type cell_get_element_type(int const cellid) const {
    ...
    return jali2tangram_elemtype(ctype);
  }
  ...
  void node_get_coordinates(int const nodeid, Point<D>* pp) const {
    ...
    assert(jp.dim() == D);
    *pp = toTangramPoint<D>(jp);
  }

 private:

  Tangram::Entity_type jali2tangram_type(Jali::Entity_type etype) const {
    static Tangram::Entity_type jali2tangram_type_[5] =
        {DELETED, PARALLEL_OWNED, PARALLEL_GHOST, BOUNDARY_GHOST, ALL};
    return jali2tangram_type_[static_cast<int>(etype)];
  }
  
  Tangram::Element_type jali2tangram_elemtype(Jali::Cell_type elemtype) const {
    static Tangram::Element_type jali2tangram_elemtype_[9] =
        {UNKNOWN_TOPOLOGY, TRI, QUAD, POLYGON, TET, PRISM, PYRAMID, HEX,
         POLYHEDRON};
    return jali2tangram_elemtype_[static_cast<int>(elemtype)];
  }
