 ...
 */
bool vdd_eq(const std::vector<Tangram::Point<2>> &a,
    const std::vector<Tangram::Point<2>> &b, const double eps = 1e-12)
{
...
void coordinates_canonical_rotation(
      const Tangram::Point<2>& center_node,
      std::vector<Tangram::Point<2>> * const xylist)
{
    int i = 0;
    auto angle = [&]() {
...
void dual_cell_coordinates_canonical_rotation(
    const Tangram::Jali_Mesh_Wrapper &mesh_wrapper,
    int const nodeid,
    std::vector<Tangram::Point<2>> * const xylist) {
  Tangram::Point<2> center_node;
...
bool faceted_poly_ok(std::vector<Tangram::Point<3>> const & fctpoints,
                     std::vector<std::vector<int>> const & facets,
                     double expected_area, double expected_volume,
                     Tangram::Point<3> expected_centroid,
                     double tol) {

  int nfacets = facets.size();

  Tangram::Point<3> gcen(0.0, 0.0, 0.0);  // Geometric center
  ...
  Tangram::Point<3> cfctcen, ctetcen;
  for (int i = 0; i < nfacets; i++) {
    int nfpoints = facets[i].size();
    for (int j = 1; j < nfpoints-1; j++) {
      Tangram::Point<3> p0 = fctpoints[facets[i][0]];
      Tangram::Point<3> p1 = fctpoints[facets[i][j]];
      Tangram::Point<3> p2 = fctpoints[facets[i][j+1]];

      Tangram::Vector<3> v0 = p1 - p0;
      Tangram::Vector<3> v1 = p2 - p0;
      Tangram::Vector<3> outnormal = cross(v0, v1);
      double fctarea = outnormal.norm()/2.0;

      cfctarea += fctarea;

      // Compute contribution to the volume by divergence theorem
      Tangram::Vector<3> vec;
      for (int k = 0; k < 3; k++)
        vec[k] = p0[k];
  ...
  Tangram::Point<3> dummy_point(-99, -99, -99);
  if (!Tangram::approxEq(expected_centroid, dummy_point) &&
      !Tangram::approxEq(expected_centroid, cfctcen, 1.0e-08)) {
  ...
  if (!Tangram::approxEq(expected_centroid, ctetcen, 1.0e-08)) {
    std::cerr << "Expected centroid " << expected_centroid[0] << " " <<
        expected_centroid[1] << " " << expected_centroid[2] << "\n";
...
TEST(Jali_Mesh, vdd_eq) {
    std::vector<Tangram::Point<2>> a, b, c;
    a = {{0.25, 0}, {0.25, 0.25}};
    b = {{0.25, 0}, {0.25, 0.25}};
    c = {{0.25, 0.25}, {0.25, 0}};
...
TEST(Jali_Mesh, coordinates_canonical_rotation) {
    std::vector<Tangram::Point<2>> xylist, xylist_canonical;
    xylist_canonical = {
                {0.75, 0.5},
                {0.75, 0.75},
                {0.5, 0.75},
...
>>>>>>> v0.9.0
    ...
    std::vector<Tangram::Point<2>> xylist;
...
>>>>>>> v0.9.0
  ...
  mesh_wrapper.cell_get_node_adj_cells(anycell, Tangram::ALL, &adjcellids);
...
>>>>>>> v0.9.0
  ...
  mesh_wrapper.dual_cell_get_node_adj_cells(center_node, Tangram::ALL,
                                            &adjdualcellids);
...
TEST(Jali_Mesh_Wrapper, Get_Exterior_Flag) {
  ...
  int nfaces = mesh_wrapper.num_entities(Tangram::Entity_kind::FACE,
                                         Tangram::Entity_type::ALL);
  for (int f = 0; f < nfaces; f++) {
    std::vector<int> fcells;
    mesh_wrapper.face_get_cells(f, Tangram::Entity_type::ALL, &fcells);
    if (mesh_wrapper.on_exterior_boundary(Tangram::Entity_kind::FACE, f))
  ...
  for (int c = 0; c < ncells; c++) {
    ...
    if (mesh_wrapper.on_exterior_boundary(Tangram::Entity_kind::CELL, c))
      ASSERT_TRUE(exterior_face_found);
    else
      ASSERT_TRUE(!exterior_face_found);
  }

  int nnodes = mesh_wrapper.num_entities(Tangram::Entity_kind::NODE,
                                         Tangram::Entity_type::ALL);
  for (int n = 0; n < nnodes; n++) {
    ...
    mesh_wrapper.node_get_corners(n, Tangram::Entity_type::ALL, &nodecorners);
    ...
    if (mesh_wrapper.on_exterior_boundary(Tangram::Entity_kind::NODE, n))
...
>>>>>>> v0.9.0
  ...
  std::vector<std::array<Tangram::Point<3>, 4>> tcoords;
...
TEST(Jali_Mesh_Wrapper, MESH_SIDES_2D) {
  ...
  int nsides_owned = mesh_wrapper.num_entities(Tangram::SIDE,
                                               Tangram::PARALLEL_OWNED);
  int nsides_ghost = mesh_wrapper.num_entities(Tangram::SIDE,
                                               Tangram::PARALLEL_GHOST);
...
>>>>>>> v0.9.0
  ...
  int ncells = mesh_wrapper.num_entities(Tangram::CELL, Tangram::ALL);
  for (int c = 0; c < ncells; ++c) {
...
>>>>>>> v0.9.0
        std::array<Tangram::Point<2>, 3> scoords;
...
<<<<<<< HEAD
        ...
        std::array<Tangram::Point<2>, 3> scoords2;
...
>>>>>>> v0.9.0
          Tangram::Vector<2> vec0 = scoords[1]-scoords[0];
          Tangram::Vector<2> vec1 = scoords[2]-scoords[0];
...
>>>>>>> v0.9.0
      std::array<Tangram::Point<3>, 4> scoords;
      mesh_wrapper.side_get_coordinates(s, &scoords);
...
<<<<<<< HEAD
      ...
      // Now get the side coordinate in the positive volume order
      std::array<Tangram::Point<3>, 4> scoords2;
      mesh_wrapper.side_get_coordinates(s, &scoords2, true);
...
>>>>>>> v0.9.0
        Tangram::Vector<3> vec0 = scoords[1]-scoords[0];
        Tangram::Vector<3> vec1 = scoords[2]-scoords[0];
        Tangram::Vector<3> vec2 = scoords[3]-scoords[0];
        Tangram::Vector<3> cpvec = cross(vec0, vec1);
...
TEST(Jali_Mesh_Wrapper, MESH_WEDGES_2D) {
  ...
  int nwedges_owned = mesh_wrapper.num_entities(Tangram::WEDGE,
                                                Tangram::PARALLEL_OWNED);
  int nwedges_ghost = mesh_wrapper.num_entities(Tangram::WEDGE,
                                                Tangram::PARALLEL_GHOST);
...
<<<<<<< HEAD
      ...
      std::array<Tangram::Point<2>, 3> wcoords;
...
<<<<<<< HEAD
      ...
      std::array<Tangram::Point<2>, 3> wcoords2;
...
>>>>>>> v0.9.0
        Tangram::Vector<2> vec0 = wcoords[1]-wcoords[0];
        Tangram::Vector<2> vec1 = wcoords[2]-wcoords[0];
        double cpvec = cross(vec0, vec1);
        double altvolume = cpvec;
        ASSERT_TRUE(altvolume*volume < 0.0);
<<<<<<< HEAD
...
<<<<<<< HEAD
      ...
      std::array<Tangram::Point<3>, 4> wcoords;
      mesh_wrapper.wedge_get_coordinates(w, &wcoords);
...
<<<<<<< HEAD
      ...
      // Now get the wedge coordinate in the positive volume order
      std::array<Tangram::Point<3>, 4> wcoords2;
      mesh_wrapper.wedge_get_coordinates(w, &wcoords2, true);
...
>>>>>>> v0.9.0
        Tangram::Vector<3> vec0 = wcoords[1]-wcoords[0];
        Tangram::Vector<3> vec1 = wcoords[2]-wcoords[0];
        Tangram::Vector<3> vec2 = wcoords[3]-wcoords[0];
        Tangram::Vector<3> cpvec = cross(vec0, vec1);
        double altvolume = dot(cpvec, vec2);
        ASSERT_TRUE(altvolume < 0.0);
<<<<<<< HEAD
...
>>>>>>> v0.9.0
  ...
  int ncorners_owned = mesh_wrapper.num_entities(Tangram::CORNER,
                                                 Tangram::PARALLEL_OWNED);
  int ncorners_ghost = mesh_wrapper.num_entities(Tangram::CORNER,
                                                 Tangram::PARALLEL_GHOST);
...
>>>>>>> v0.9.0
  ...
  int nnodes = mesh_wrapper.num_entities(Tangram::NODE, Tangram::ALL);
  for (int n = 0; n < nnodes; ++n) {
    std::vector<int> corners;
    mesh_wrapper.node_get_corners(n, Tangram::ALL, &corners);
<<<<<<< HEAD
...
TEST(Jali_Mesh_Wrapper, MESH_NON_DEFAULT_OPTS) {
  ...
  Tangram::Jali_Mesh_Wrapper mesh_wrapper1(*mesh, request_sides, request_wedges,
                                           request_corners);

  ASSERT_GT(mesh_wrapper1.num_entities(Tangram::SIDE, Tangram::ALL), 0);
  ASSERT_EQ(mesh_wrapper1.num_entities(Tangram::WEDGE, Tangram::ALL), 0);
  ASSERT_EQ(mesh_wrapper1.num_entities(Tangram::CORNER, Tangram::ALL), 0);
...
>>>>>>> v0.9.0
  ...
  Tangram::Jali_Mesh_Wrapper mesh_wrapper2(*mesh, request_sides, request_wedges,
                                           request_corners);

<<<<<<< HEAD
  ASSERT_GT(mesh_wrapper2.num_entities(Tangram::SIDE, Tangram::ALL), 0);
  ASSERT_GT(mesh_wrapper2.num_entities(Tangram::WEDGE, Tangram::ALL), 0);
  ASSERT_EQ(mesh_wrapper2.num_entities(Tangram::CORNER, Tangram::ALL), 0);
...
>>>>>>> v0.9.0
  ...
  Tangram::Jali_Mesh_Wrapper mesh_wrapper3(*mesh, request_sides, request_wedges,
                                           request_corners);

<<<<<<< HEAD
  ASSERT_GT(mesh_wrapper3.num_entities(Tangram::SIDE, Tangram::ALL), 0);
  ASSERT_GT(mesh_wrapper3.num_entities(Tangram::WEDGE, Tangram::ALL), 0);
  ASSERT_GT(mesh_wrapper3.num_entities(Tangram::CORNER, Tangram::ALL), 0);
...
>>>>>>> v0.9.0
  ...
  Tangram::Jali_Mesh_Wrapper mesh_wrapper4(*mesh, request_sides, request_wedges,
                                           request_corners);

<<<<<<< HEAD
  ASSERT_EQ(mesh_wrapper4.num_entities(Tangram::SIDE, Tangram::ALL), 0);
=======
>>>>>>> v0.9.0
  ASSERT_EQ(mesh_wrapper4.num_entities(Tangram::WEDGE, Tangram::ALL), 0);
  ASSERT_EQ(mesh_wrapper4.num_entities(Tangram::CORNER, Tangram::ALL), 0);
...
TEST(Jali_Mesh_Wrapper, MultiCell_Facetization) {
  ...
  Tangram::Jali_Mesh_Wrapper mesh_wrapper(*mesh, request_sides, request_wedges,
                                         request_corners);
  ...
  std::vector<Tangram::Point<3>> fctpoints;
  ...
  for (int n = 0; n < nnodes; n++) {
    ...
    mesh_wrapper.node_get_cells(n, Tangram::Entity_type::ALL, &ncells);
    if (ncells.size() == 8) {
      if (nproc == 1) {
        inode = n;
        break;
        ...
        for (int i = 0; i < ncells.size(); i++) {
          Tangram::Entity_type ctype = mesh_wrapper.cell_get_type(ncells[i]);
          if (ctype == Tangram::Entity_type::PARALLEL_OWNED)
            owned_found = true;
          else
            ghost_found = true;
        }
...
TEST(Jali_Mesh_Wrapper, Skewed_2DCell_Geometry) {
  ...
  double carea = 1.25;
  Tangram::Point<2> cen(0.58333333333333333, 0.58333333333333333);
  ...
  Tangram::Point<2> cen_computed;
  mesh_wrapper.cell_centroid(0, &cen_computed);
  ASSERT_TRUE(Tangram::approxEq(cen, cen_computed, 1.0e-8));
}
...
TEST(Jali_Mesh_Wrapper, Skewed_3DCell_Geometry) {
  ...
  // facets use a different face point than Tangram mesh wrapper. So
  // don't compare this
  double cfarea = -1.0;
